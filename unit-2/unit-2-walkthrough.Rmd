...
once enjoyed bipartisan support, we'll see in this walkthrough how these
standards have become a political punching bag, particularly around the
...
briefly examine next is an exemplary illustration of these four defining
...
\#COMMONCORE Project provides a detailed discussion of the methods used
to arrive at the conclusions in *\#commoncore: How social media is
changing the politics of education*. Provided below is a summary of how
authors retrieved the Twitter data and the analyses for each of the five
acts in the website. I highly encourage you to take a look at this
section if you'd like to learn more about their approach and in
particular if you're unfamiliar with how users can interact and
...
ran a community detection algorithm to identify and represent structural
sub-communities, or factions (a "faction" in this sense is a group with
more ties within than across group even those group boundaries are
somewhat porous).
...
Some of the techniques and approaches used in this study, however, you
are likely already familiar with high. For example, take a quick look at
the [*Explore the Networks*](https://www.hashtagcommoncore.com/#2-1)
...
If you are not interested in setting an account of your own, I can
provide you with temporary keys and tokens to download data from Twitter
and have also provided data for this walkthrough in the data folder.
...
data from web pages, which explore in Module 2 later this year, APIs are
considerably easier to work with than the HTML or XML data, ensure that
data is collected legally, and with R packages like `rtweet`, are
logistically easier to work with, as we'll see at the workshop this
summer.

For a more in-depth explanation of APIs, how they work, and a preview of
some of the `rtweet` functions we'll be using this summer, we highly
...
10. To make sure it works, you can run the following code and check to
...
Use the code chunk below to wiew your new `ccss_tweets` data frame using
...
function using the `%>%` operator and specifying the `lang` as the
column whose values we want to count and sorting from high to low:
...
towards the end of our analysis, let's limit our tweets to just those in
...
Recall from Unit 1 that the first two columns in an edglist should
...
features similar and in addition to those found in the {igraph} package.
...
measures, at many others have been developed and can be calculated in
most common social network-analysis software applications. In fact,
...
{ggraph} package for creating attractive network
...
"pipe" functions together to progressively wrangle data, ggraph and
...
while others for undirected graphs.
...
In this section, we'll introduce and apply the {vader} package to gain
some
