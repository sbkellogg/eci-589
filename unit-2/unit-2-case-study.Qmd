...
educators](https://github.com/sbkellogg/eci-589/blob/main/unit-2/lit/sna_mooc_irrodl_bjet_articles.pdf). *International
...
Both {tidygraph} and {ggraph} depend heavily on the [igraph network
...
Use the code chunk below to create a sociogram for the DLT 2 MOOC
...
a sociogram based on degree, so those actors with more connections stand
...
Run the following code to create a new variable called `in-degree` (i.e.
number of incoming replies each actor received) to our DLT 1 network and
assign the output `dlt1_network` again so the results are saved to our
network object:
...
Now let's temporarily convert our "activated" nodes to a simple table
output, or "[tibble](https://tibble.tidyverse.org)," using the
`as_tibble()` function. This will allow us to get a better look at our
nodes and the measure of `degree` instead of `local_size()` that we just
created.
...
others, all information shared is visible to all participants.
...
both directions. It is commonly used to determine how inter-connected
directed networks are. The tendency towards reciprocity in most
networks, sometimes referred to as the "norm" of reciprocity, has been
acknowledge by ancient writers such as Cicero who stated:
...
reciprocated ties is somewhat low in our DLT 1 network; but again, the
size of our network is rather large and should guide our judgement about
this value. From an interpretation standpoint, however, this measure is
simply the proportion of outgoing edges that also have an incoming edge.
That is, approximately 20% of ties in our network are symmetrical, or
reciprocated.

And if we wanted to highlight this lack of reciprocity in a network
graph, we could use the
...
this phenomenon would predict that if Teacher A collaborates with
Teacher B and Teacher B collaborates with Teacher C, then Teacher A is
more likely at some point to collaborates with Teacher C and "complete"
the triangle and less likely to collaborate with Teacher E since they do
...
For our particular network, a very small value of .088 is not too
...
How do network-level measures for DLT 1 and DLT 2 discussion compare?
...
We will not explore the use of models for SNA until Unit 4, but recall
