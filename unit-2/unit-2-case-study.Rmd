...
As Carolan @carolan2014 notes in SNA and Education:
...
researchers utilize an array of algorithms and indices to characterize
different features of social networks. These algorithms and indices rely
on some basic---and not-so-basic---mathematical computation.
...
introduce some key functions from the the {igraph} package for
characterizing networks as a whole.
...
surrounding the Common Core State Standards,
...
The concepts and measures that are introduce in this section include
some of the most common ways in which features of complete networks are
described, as well was.
...
In a network of 17 students, it is not hard to imagine that the pattern
of who studies with whom, for example, would look much different than if
the network consisted of 200 students. Size affects other network
measures, but on a conceptual level, it influences the structure of
relations, as actors only have so many resources and capacities for
creating and maintaining ties with others. Also, in smaller networks, it
is easier for actors to know each other. In formal terms, size is simply
a measure of the number of nodes in the network. In the case of the
Fraternity Data, it is not a terribly interesting property, as it simply
reflects the network's boundary.
...
is network size, and λ is the maximum number of alters requested or
