...
this unit we move beyond visual depictions of networks using sociograms
from our previous SNA case study and learn to describe networks using a
wide range network-level measures. Specifically, our analyses are guided
by research conducted by Daly and Finnagan @daly2011ebb on the The ebb
and flow of social network ties between district leaders under
high-stakes accountability. As we work through step of the
data-intensive workflow [@krumm2018], we will focus on the following
topics:
...
Use the code chunk below to load the Let's go ahead and load the
tidyverse, igraph, tidygraph, ggraph, and readxl packages that we'll be
using in this case study:
...
reshaping, transforming, and merging data (Wickham and Grolemund 2016).
As highlighted in Estrellado et al. (2020), wrangling network data can
be even more challenging than other data sources since network data
often includes variables about both individuals and their relationships.
For our data wrangling in this unit, we're keeping it simple since
working with relational data, especially in the form of matrices, is a
bit of a departure from our working with rectangular data frames.
...
You may have seen warning stating: Setting row names on a tibble is
deprecated. You can ignore that for now but it's basically telling us
these functions old we we need to use newer function or our code will
some day stop working.
...
For our particular network, a value of 24.4% and is not too surprising
...
tools to examine network change over time [@carolan2014]. These models
are use to empirically test not just if changes in key structure
measures are statistically significant, but also explain potential
...
1.  What method(s) did Daly and Finnagan use in their longitudinal case
...
1.  **Select.** Select a several network-level measures that you think
    would be interesting or relevant for our target audience and that
...
write your narrative in the space that follows.
