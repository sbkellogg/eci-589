...
You may have seen a warning stating: `Setting row names on a tibble is deprecated.` You can ignore that for now but it's basically telling us these functions are old and we need to use newer functions or our code will some day stop working.
...
Much better! Now we can see that student 1 indicated that student 2 is their friend, and student 2 indicated that student 1 is their friend, so we can say that this friendship is "reciprocated" or "mutual." As we'll see in Unit 2, reciprocity is an import network-level measure in SNA.
...
The word "class" and "object" have been used quite a bit in this case-study and warrant a brief explanation. Classes and objects are basic concepts of Object-Oriented Programming environments like R.
...
Check your data folder to see that it is indeed saved there. You should now see a new .csv file in addition to the Excel files.
...
One thing to keep in mind when building a network viz with {ggraph}, is that just like it's `ggplot()` counterpart introduced in the [Data Visualization Basics](https://posit.cloud/learn/primers/1.1) primer, there is a minimal code template for producing a basic plot. Specifically, ggraph requires 3 main functions:
...
That's not much better so let's stick with the "stress" layout for now. Feel free to try out some other [ggraph layout methods](https://ggraph.data-imaginist.com/articles/Layouts.html) if you like, however.
...
Much better! And notice again how we shifted the `geom_node_point()` layer of our graph to after the `geom_edge_link()` so the parts of nodes would not be hidden under the edges.
...
Congratulations - you've completed the Unit 1 case study!
