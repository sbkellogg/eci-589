...
RStudio Cloud workspace. Click the green arrow to run the
`sessionInfo()` function and take a look at the "attached" packages:

```{r}
sessionInfo()
```

You should see some familiar tidytext packages from out like {dplyr} and
{readr} which we'll be using shortly. You should also see an important
package call {igraph} that will rely on heavily for our network
analyses.

Note that you can also manually check the Files, Plots, & Packages Pane
in the lower right hand corner of RStudio Cloud to make sure these
...
For our data wrangling this week, we'll keep it simple
...
b.  **Create a Network Object**. We focus on tools from
    the `dplyr` package to view, `rename`, select, slice, and filter our
