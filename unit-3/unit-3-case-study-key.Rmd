...
network and use the familiar `mutate()` function to create a new
`strong_omponent` variable that indicates and saves the strong
components to which each node belong:
...
Let's take a look now at the nodes in our new `dlt1_network`, which
should now contain a our new variable. To do so, we'll use another handy
function from the {tidygraph} package, `as_tibble()`, that will convert
our node list to a standard table that will allow us to view every row
in our node list:
...
Similar to our graph of weak components, we see this network as a strong
component with many members, and the remaining components are all
isolated nodes.
...
entire suite of {tidyverse} packages to for analyzing nodes and edges in
a network! We'll be using the activate function again in section [3b.
...
As you probably noted above, the `clique_num()` function does not take
into account directionality of our ties. It looks like we have at least
one dense cluster, however, of 8 nodes that all have ties to one
another.
...
Let's see if there are any cliques that contain at least 8 nodes?
...
look. Because this is a bit computationally intensive, this make take a
minute or so to run.
...
the far end a new `subgroup`variable that includes and ID number
...
In this section, we shift out analytical lens from groups to individual
actors and the pattern of relations in which they are embedded. Implicit
...
Recall from our previous unit focused on complete network analysis that
one simple and often ignored structural property of a social network is
its size. This also applies to egocentric analysis.
...
between dyads are counted more than once. Alternatively, we could
...
be a measure of frequency of replies between actors.

Let's dust off our
[`ggplot()`](https://ggplot2-book.org/getting-started.html#basic-use)
skills and take a look at the distribution of `out_degree` for each
ego's local neighborhood by using in `geom_histogram()` function for
creating histograms.
...
While, we could calculate invdiivudally the mean and variance for each
egocentric measure created above using the `summarise()` function
demonstrated earlier, but of course R has a package and function for
...
the case since for every tie that go "in" there is always one going
"out."
...
United States and calculated compositional and variance measures for
...
to the nodes in our network and answer the questions that follow. Hint:
you may want to look at the [functions to calculate node and edge
...
[@carolan2014]. The ultimate goal of using information about a network's
substructures is test whether there is a relationship between these
measures and educational outcomes of interest.
