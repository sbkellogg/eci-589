...
need to convert our directed network to an undirected network, or
"symmetrize" our network as discussed in Carolan @carolan2014. Prior to
activating our nodes and creating a new variable indicating group
membership, we will need to use the `to_undirected()` function to
convert our network from directed to undirected.

Run the following code to group our nodes based on their edge
betweenness and print our new `dlt1_clusters` object and take a quick
look:

```{r}
dlt1_clusters <- dlt1_network |>
  to_undirected() |>
  activate(nodes) |>
