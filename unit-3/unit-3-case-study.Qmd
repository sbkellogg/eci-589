...
Applications that the following four features used by
\@freeman2004development to define the social network perspective:
...
to an approach we use in ECI 588 Text Mining in Education, [*Twitter
...
Networks](https://www.hashtagcommoncore.com/#2-1) section from Act 2:
Central Actors and the Transmitters, Transceivers and Transcenders.
...
-   ***Transcenders*** is measured by *degree* and consists of actors
    (i.e. Twitter uers) who have both high out-degree, as well as having
    high in-degree.
...
For Unit 3, we are going to focus our questions on something a bit less
...
To address the last question, we'll introduce the {VADER} package for
sentiment analysis in our modeling section.
...
In our ECI 589 SNA and Education Workspace on Posit Cloud, an RStudio
Project titled `Unit 3 - Case Study` has already been set up for you and
includes a `.Rproj` file in the main directory in the Files pane. Go
there and see if you can locate it.
...
see which packages have already been installed and loaded into Posit
...
a.  **Import Data**. We revisit the `read_csv()` function from the
    tidyverse {readr} package to read the `ccss-tweets-fresh.csv` file
    from the data folder.

b.  **Format Network Data**. We shape our data files into two common
    formats for storing network data: edgelists and nodelists.
...
To help keep things simple, we'll be using a data download from twitter
that only includes relatively small set of tweets from a single week.
...
since we are using some very basic techniques for visualization.
...
Recall that the **edgelist** format is slightly different than other
formats you have likely worked with in that the values in the first two
...
Note that we also use R's new built in `|>` pipe operator, which serves
the same purpose as the older tidyverse `%>%` pipe operator.
...
The {tidytext} package has a very handy `unnest_tokens()` function to
...
function from the tidyverse {dplyr} package, an updated version of the
`gather()` function introduced in the [RStudio Reshape Data
...
Recall that the {tidygraph} package contains a `tbl_graph()` function
...
indicates that a connection or tie is not necessarily reciprocated or
mutual. For example, our network is directed because even though a user
may follow, mention or reply to someone else, they may not necessarily
receive a follow, mention or reply back. A facebook friendship network
is undirected, however, since someone can not "friend" someone on
Facebook without them also indicating a "friend" relationship, so the
friendship is mutual or reciprocated.
...
One issue with this {igraph} output, and that the {tidygraph} package is
great at solving, is identifying and saving as a new variable who
belongs to which component. We can sort of see from above that the most
twitter users in our network belong to component 11.
...
Now take a look at the nodes in our new `ccss_network`, which should now
contain our new variable.
...
"rdsathene" exceeds the total number of nodes in our network and are
probably wondering how that is possible. Recall that our network is
currently treated at a "multigraph," indicating that some dyads have
multiple edges. Therefore, some edges between dyads are counted more
than once.
...
skills by completing the following code to create a histogram that shows
the distribution of `in_degree` (i.e. the number of replies or mentions
a user has received) for Twitter users in our network.

```{r histogram}
ccss_network |> 
  as_tibble() |>
  _____() +
  ____________(aes(x = _______))
```
...
As you can see, roughly half the egos in the network received about 5
...
While we could calculate individually the mean and variance for each
...
Just like our {tidygraph} package, the {skimr} package also plays nicely
...
1.  Which two actors have the highest betweenness?
...
measures available in the {igraph} and {tidygraph} packages. Feel free
to explore [other functions to calculate node and edge
centrality](https://tidygraph.data-imaginist.com/reference/centrality.html)
...
and it will show in the Viewer pane and then you can enlarge and even
...
highlight the main "transcenders" or "transceivers" in our network. See
if you can also make your plot either more "aesthetically pleasing"
and/or more purposeful in what it's trying to communicate.
...
This clearly does not provide much insight, nor is it any improvement to
our previous sociogram, but it definitely is colorful!!
...
social networks until Unit 4, but take a look at [The Mood of the Common
Core Factions](https://www.hashtagcommoncore.com/#4-2) section from the
...
**Note:** The {vader} package can take quite some time to run on large
datasets, but the example shown with our small subset of tweets should
run fairly quickly.
...
Unlike other approaches demonstrated in ECI 588, which assign sentiment
to each individual word in a corpus of tweets, the VADER assigns a
number of different sentiment measures based on the context of the
entire social-media post, or in our case a tweet. This saves quite a bit
of data wrangling allowing us to work with our tweets in their original
format.

VADER basically has just one function, `vader_df()` that does one thing
and expects just one column from one data frame. He's very single
minded! Let's give VADER our `ccss_tweets` data frame we created in the
...
Not quite as bleak as we might have expected according to something with
the name VADER!
...
insights about the the Twitter datasets included in your data folder or
...
some of your new R skills. To do so, you will need to "render" your
document by clicking the ![](img/render.png){width="4%"} Render button
in the menu bar at that the top of this Quarto file. This will do two
things; it will:

1.  check through all your code for any errors; and,

2.  create a file in your directory that you can use to share you work
    through [RPubs](https://rpubs.com/about/getting-started) (see
    screenshot below to publish), [GitHub
    Pages](https://pages.github.com), [Quarto
    Pub](https://quarto.org/docs/publishing/quarto-pub.html), or any
    [other methods](https://quarto.org/docs/publishing/).
