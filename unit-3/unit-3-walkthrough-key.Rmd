...
3.  **Explore**: In section 3, we use the {tidygraph} package and the
    companion {ggraph} package once again to calculate basic summary
    stats for our network and will try to replicate and ideally improve
    upon the sociogram in our course text.

4.  **Model**: We wrap up our analysis in Section 4 by introducing
    trying to replicate the ergm models used in Chapter 9 of
    @carolan2014 and checking to see how well our model "fits our data"
    and examining diagnostics that might indicate our model has issues.
...
Chapter 9 of @carolan2014 in order to un
...
Chapter 9 are the quadratic assignment procedure (QAP) and Exponential
...
These are illustrated by research questions 2 & 3 from above.
...
For our data wrangling this week, we'll be . Our primary goals for Unit
1 are learning how to:

a.  **Import Matrices**. In this section, we introduce
...
names to our columns and rows for adjacency matrix in just a bit.
...
tie is a directed valued "arc" where the value indicates the strength of
the relationship.
...
or column names, assign the matrix to a variable named `leader_matrix`:
...
Take a look at the matrix you imported and answer the questions that
follow:
...
little more complex that binary networks, which indicate just the
presence or absence relationship between two actors, we will need to
"dichotomize" our matrix, which in practice entails converting our
valued matrix to binary matrix with cells containing just 1s or 0s.

**However,** before we can convert dichotomize our "matrix," we first
...
To dichotmize our our matrix, the following code will assign 0s to all
values in our matrix that are less than or equal to 2, and 1s to all
values that greater the or equal to 3:
...
## 2c. Create Graph with Attributes
...
The {igraph} package introduced in Unit 1 has a convenient function for
extracting an edge list from a matrix, but first we need to convert our
matrix to an igraph graph object.
...
Note that we also included the diag = FALSE argument which converts all
values along the diagonal to 0s, thereby removing our self-loops. I
believe they may have been included when calculating the network
descriptives included in Table 9.1, and our own descriptives may not
match exactly, but we'll need to remove these from our network anyways
for the ERGM analysis so this will save us a step.
...
Recall from Unit 2 that we introduce the {tidygraph} package for
preparing and summarizing our Twitter network. Tidygraph incudes the
...
In Section 3, we use the {tidygraph} package again for retrieving
network descriptives and the {ggraph} package to create a network
...
As noted in SNA and Education, many analyses of social networks are
primarily descriptive and aim to either represent the network's
underlying social structure through data-reduction techniques or to
...
Recall **degree** is the number of ties to and from an ego. In a
directed network, in-degree is the number of ties received, whereas
out-degree is the number of ties sent.
...
We now see that these in-degree and out-degree have been added to the
nodes in our network in addition to the previously included addtributes.
...
9.1. For example, our average efficacy and trust scores were 6.65 and
4.78 respectively, compared to 6.64 and 4.77 as reported in Chapter 9.

The average in/out-degree for our school leaders data is 3.26, however,
which is a little lower than that reported by Carolyn. Again, this may
be a function of our removal of self-loops perhaps. Try changing the
`diag = FALSE` argument added to the `graph.adjacency()` function above
to `TRUE` to include self-loops and rerunning all the code above. Are
these stats closer to those reported by Carolyn?
...
In Chapter 9, a directed and dichotomous sociogram of the
**collaboration** network for year 3 is shown in Figure 9.1.
...
> **Exponential Random Graph Models** provide a way to determine whether
...
a the `x =` and `vertices =` attributes in the `as.network()` function
...
Let's start by running a simple model both to demonstrate the most
commonly used functions for ERGMs and to replicate the approach in
...
Since `ergm` automatically uses a stochastic MCMC-based estimation
...
confidential exchange tie in year 3 is much smaller than 0.50. The
reciprocity parameter, on the other hand, is 3.11 in our model, and
indicates a strong tendency for confidential-exchange ties to be
reciprocated.

Now let's add the an ergm-term for transitivity. Modeling transitivity,
or the "friend of a friend" phenomenon in social networks, is both
...
As you can see, we have 202 transitive triads.
...
is likely to occur between two people who both confide in another
person.
...
behavioral (e.g., class attendance) characteristics. Two such attributes
examined in Chapter 9 were gender and leadership efficacy as measured by
survey items based on the [Principal Efficacy
...
while sometimes prone to model degeneracy without some model fine tunig.
To simplify this approach, I've include these attributes as using the
...
likely to either send or receive at tie.
...
Although there appears to be a slight, though not statistically
significant tendency towards male leaders being more likely to send or
relieve ties, the primary drivers of network formation appear to be
structural features of the network including reciprocity and
transitivity.

Sadly, we were not able to replicated the findings from @carolan2014,
but we did not replicate the model exactly, nor did we fine tune our
model parameters. Regardless, this provided a practical exercise for
demonstrating ergm models.
...
The`gof()` function is a bit different than the `summary()` and `ergm()`
functions, in that it only takes 3 ergm-terms as arguments: degree, esp
(edgwise share partners), and distance (geodesic distances). Each of
these terms captures an aggregate network distribution, at either the
node level (degree), the edge level (esp), or the dyad level (distance).
...
too look for "balance." The charts with all the squigglies, to use a
non-technical term, should have a scale that ideally is centered on 0
and be roughly straight line running through it, while the chart that
...
indicate there is room for improvement but nothing is wildly off.
...
you are interested in exploring, could you investigate using these
approaches?
